...
#include "petalo_source/persistency/HDF5Writer.h"
...
PersistencyManager::PersistencyManager(G4String init_macro,
  ...
  BasePersistencyManager(), msg_(0), init_macro_(init_macro), macros_(macros),
...
G4bool PersistencyManager::Store(const G4Event* event)
{
  ...
  if (!store_evt_) {
    nexus::TrajectoryMap::Clear();
    if (store_steps_) {
      nexus::SaveAllSteppingAction* sa = (nexus::SaveAllSteppingAction*)
...
void PersistencyManager::StoreTrajectories(G4TrajectoryContainer* tc)
{
  ...
  for (size_t i=0; i<tc->entries(); ++i) {
    nexus::Trajectory* trj = dynamic_cast<nexus::Trajectory*>((*tc)[i]);
    if (!trj) continue;
...
void PersistencyManager::StoreHits(G4HCofThisEvent* hce)
{
  ...
  for (auto i=0; i<hct->entries(); i++) {
    ...
    if (hcname == nexus::IonizationSD::GetCollectionUniqueName())
      StoreIonizationHits(hits);
    else if (hcname == nexus::PmtSD::GetCollectionUniqueName()) {
      StorePmtHits(hits);
    } else {
      G4String msg =
...
void PersistencyManager::StoreIonizationHits(G4VHitsCollection* hc)
{
  ...
  nexus::IonizationHitsCollection* hits =
    dynamic_cast<nexus::IonizationHitsCollection*>(hc);
  if (!hits) return;
  ...
  for (size_t i=0; i<hits->entries(); i++) {
    ...
    nexus::IonizationHit* hit = dynamic_cast<nexus::IonizationHit*>(hits->GetHit(i));
    if (!hit) continue;
...
void PersistencyManager::StorePmtHits(G4VHitsCollection* hc)
{
  ...
  if (sensdet_it == sensdet_bin_.end()) {
    for (size_t j=0; j<hits->entries(); j++) {
      nexus::PmtHit* hit = dynamic_cast<nexus::PmtHit*>(hits->GetHit(j));
      if (!hit) continue;
  ...
  for (size_t i=0; i<hits->entries(); i++) {
    ...
    nexus:: PmtHit* hit = dynamic_cast<nexus::PmtHit*>(hits->GetHit(i));
    if (!hit) continue;
...
G4bool PersistencyManager::Store(const G4Run*)
{
  ...
  nexus::NexusApp* app = (nexus::NexusApp*) G4RunManager::GetRunManager();
