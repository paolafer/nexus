...
namespace nexus {
  ...
  PetPlainDiceCheck::PetPlainDiceCheck(): 
    BaseGeometry(),
    rows_(8),
    columns_(8),
    visibility_ (1),
  ...
  void PetPlainDiceCheck::SetSize(G4double xsize, G4double ysize)
  {
  xsize_ = xsize;
  ysize_ = ysize;
  }

  void PetPlainDiceCheck::Construct()
  {
  ...
  G4ThreeVector PetPlainDiceCheck::GetDimensions() const
  {
    return dimensions_;
  }
  
  const std::vector<std::pair<int, G4ThreeVector> >& PetPlainDiceCheck::GetPositions()
  {
    return positions_;
  }

  // void PetPlainDiceCheck::SetMaterial(G4Material& mat)
  // {
  //   _out_mat = mat;
  // }
