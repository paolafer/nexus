// ----------------------------------------------------------------------------
// nexus | FullRingLYSO.cc
//
// This class implements the geometry of a cylindric ring of LYSO,
...
namespace nexus {
  ...
  {
     // Messenger
    msg_ = new G4GenericMessenger(this, "/Geometry/FullRingLYSO/",
                                  "Control commands of geometry FullRingLYSO.");
    ...
    msg_->DeclareMethod("pointFile", &FullRingLYSO::BuildPointfile, "Location of file containing distribution of event generation points.");
  ...
  void FullRingLYSO::BuildCryostat()
  {
    ...
    G4Tubs* LYSO_solid =
      new G4Tubs("LYSO", inner_radius_ - kapton_thickn_,
    ...
    LYSO_logic_ =
      new G4LogicalVolume(LYSO_solid, LYSO, "LYSO");
    new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), LYSO_logic_,
		      "LYSO", cryostat_logic, false, 0, true);
    ...
    new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), active_logic_,
		      "ACTIVE", LYSO_logic_, false, 0, true);
    ...
    new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), kapton_int_logic,
		      "KAPTON", LYSO_logic_, false, 0, true);
    ...
    new G4PVPlacement(0, G4ThreeVector(0., 0., z_pos), kapton_lat_logic,
    		      "KAPTON", LYSO_logic_, false, 0, true);
    new G4PVPlacement(0, G4ThreeVector(0., 0., -z_pos), kapton_lat_logic,
    		      "KAPTON", LYSO_logic_, false, 1, true);
  ...
  void FullRingLYSO::BuildSensors()
  {
 ...
 void FullRingLYSO::BuildPhantom()
  {
    phantom_diam_ = 6.*cm;
  ...
  G4ThreeVector FullRingLYSO::GenerateVertex(const G4String& region) const
  {
    ...
    if (region == "CENTER") {
      ...
      if (i == (sensitivity_vertices_.size()*events_per_point_-1)) {
       G4Exception("[FullRingLYSO]", "GenerateVertex()",
	...
	auto pos = i % sensitivity_vertices_.size();
        ...
        G4Exception("[FullRingLYSO]", "GenerateVertex()", FatalErrorInArgument, "Sensitivity point out of range.");
      }

    } else {
      G4Exception("[FullRingLYSO]", "GenerateVertex()", FatalException,
                  "Unknown vertex generation region!");
    }
    return vertex;
  }

  G4int FullRingLYSO::binarySearchPt(G4int low, G4int high, G4double rnd) const {
  ...
  G4ThreeVector FullRingLYSO::RandomPointVertex() const
  {
  ...
  void FullRingLYSO::BuildPointfile(G4String pointFile)
  {
    int Nx, Ny, Nz;
    float Lx, Ly, Lz;
  ...
  void FullRingLYSO::CalculateSensitivityVertices(G4double binning)
  {
