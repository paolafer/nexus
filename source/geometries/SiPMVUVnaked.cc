// ----------------------------------------------------------------------------
 ...
 namespace nexus {
   ...
   SiPMVUVnaked::SiPMVUVnaked(): BaseGeometry(),
                             visibility_(1),
                             eff_(1.),
   ...
   SiPMVUVnaked::~SiPMVUVnaked()
   {
   }

   void SiPMVUVnaked::Construct()
   {
     ...
     G4Box* sipm_solid = new G4Box("SiPMVUVnaked", sipm_x/2., sipm_y/2., sipm_z/2);

     G4Material* plastic = MaterialsList::FR4();
     G4LogicalVolume* sipm_logic =
       new G4LogicalVolume(sipm_solid, plastic, "SiPMVUVnaked");
     ...
     G4String sdname = "/SIPM/SiPMVUVnaked";
     G4SDManager* sdmgr = G4SDManager::GetSDMpointer();

     if (!sdmgr->FindSensitiveDetector(sdname, false)) {
       ToFSD* sipmsd = new ToFSD(sdname);
       if (sensor_depth_ == -1)
         G4Exception("[SiPMVUVnaked]", "Construct()", FatalException,
